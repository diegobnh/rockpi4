...
logging.basicConfig(filename="output", format='%(message)s',
...
# this is the minimum accuracy to avoid continue try more iterations
...
class Common:
    ...
    def remove_lines_zero_values(file_path):
        ...
        df = pd.read_csv(file_path)
        new_df = df.drop(
            df[df.eq(0).sum(axis=1) > maximum_zero_per_line].index)
        return new_df
    ...
    def print_performance(list_scores, list_pmcs):
        ...
        for i in range(len(list_pmcs)):
            ...
            print("Mean Accuracy: {0:.2f} ".format(
                round(list_scores[i], 2)), my_formatted_list)
        print("Final Mean : {0:.2f} ".format(
            round(sum(list_scores)/len(list_scores), 2)))
    ...
    def plot_boxplot(model_name, scores_list, mean):
        ...
        file_name = model_name+"_" + \
            sys.argv[4]+"_"+"Kfold_" + \
            str(K_FOLD_N_SPLITS)+"_Mean_"+str(mean)+"_FINAL"+".png"
    ...
    def plot_barplot(model_name, scores_list, mean, i):
        ...
        file_name = model_name+"_"+sys.argv[4]+"_"+"Kfold_"+str(
            K_FOLD_N_SPLITS)+"_"+"Iteration_"+str(i)+"_Mean_"+str(mean)+".png"
    ...
    def drop_constant_column_v2(df):
        ...
        drop_col = [str(i) for i in range(len(df.columns))
                    if df[df.keys()[i]].nunique() == 1]
        #print("Columns to remove:",drop_col)
        # print(df.columns)
    ...
    def plot_density_histogram(models, best_pmcs):
        ...
        for i in range(len(models)):
            ...
            for j in range(len(best_pmcs[i])):
                ...
                plt.xlabel('Values')
                plt.ylabel('Density')
                name_file = "Density_histogram_" + \
                    models[i][0]+"_"+str(best_pmcs[i][j])+".png"
    ...
    def plot_density_histogram_for_correlations(models, best_pmcs, correlation):
        ...
        for i in range(len(models)):
            ...
            for j in range(len(best_pmcs[i])):
                ...
                plt.xlabel('Values')
                plt.ylabel('Density')
                name_file = "Density_histogram_"+correlation+"_" + \
                    models[i][0]+"_"+str(best_pmcs[i][j])+".png"
    ...
    def export_tree(df, clf):
        ...
        last_column_name = list(df_pmcs_avg.columns.values)[-2:]
        X = df_pmcs_avg.loc[:,
                            df_pmcs_avg.columns.difference(last_column_name)]
    ...
    def get_features_and_target(df):
        last_column_name = list(df.columns.values)[-2:]
        #print("Targets:", last_column_name)
        # print(type(df.columns))
        X = df.loc[:, df.columns.difference(last_column_name)].values
        Y = df[last_column_name].to_numpy()
        return X, Y
...
class Models:
    ...
    def get_models_default_parameters():
        models = [("DTR", MultiOutputRegressor(DecisionTreeRegressor(random_state=0))),
                  # ("RFR",RandomForestRegressor(random_state=0,n_estimators=30,n_jobs=-1)), #, n_jobs=-1)),
                  ("GBR", MultiOutputRegressor(
                      GradientBoostingRegressor(random_state=0, n_estimators=30))),
                  ("XGBR", XGBRegressor(random_state=0, objective='reg:squarederror', colsample_bytree=0.9, max_depth=None, n_estimators=30, n_jobs=-1))]  # ,n_jobs=-1))] #,
        # ("MLP",MLPRegressor(max_iter=5000))]
        return models

    @staticmethod
    def get_models_initial_parameters():
        models = [("DTR", MultiOutputRegressor(DecisionTreeRegressor(max_depth=30, max_features='auto'))),
                  #("RFR",RandomForestRegressor(bootstrap='True', max_depth=None, max_features=None, n_estimators=10,n_jobs=-1)),
                  ("GBR", MultiOutputRegressor(
                      GradientBoostingRegressor(random_state=0, n_estimators=30))),
                  ("XGBR", MultiOutputRegressor(XGBRegressor(random_state=0, objective='reg:squarederror', colsample_bytree=0.9, max_depth=None, n_estimators=30, n_jobs=-1)))]  # ,
...
class HCA:
    ...
    def create_hca():
        ...
        for corr_method in correlations:
            ...
            corr_condensed = shc.distance.squareform(
                corr)  # convert to condensed
            # orientation="right"
            z = shc.linkage(corr_condensed, method='average')
            # if orientation right, change rotation to 0
            dendrogram = shc.dendrogram(
                z, labels=corr.columns, orientation="right", leaf_rotation=0, leaf_font_size=10)
    ...
    def get_features_per_cluster(number_of_clusters, corr_method):
        ...
        for i in range(len(df_pmcs.columns)):
            ...
            feat_per_cluster[cluster_id].append(df_pmcs.columns[i])

        # Now we need to get the correlation
        dictionary = Filter.get_features_correlated_with_target_sorted(
            ds.df_pmcs_avg.copy(), corr_method)
        print(dictionary)
        example = []
        for key in dictionary.keys():
            for i in range(number_of_clusters):
                for j in range(len(feat_per_cluster[i])):
                    #############################################################################
                    # How do I decide which label's correlation to go with?
                    # 2 different labels means 2 different sets of correlations,
                    #############################################################################
                    # feat_correlation[i].append(dictionary.get(feat_per_cluster[i][j]))
                    feat_correlation[i].append(dictionary[key].transpose().get(
                        feat_per_cluster[i][j]).values[0])

            example.append(feat_correlation)
        print()
        print("Are the clusters equal for each target?",
              example[0] == example[1])
        ...
        features_correlation_per_cluster = [[]
        ...
        for i in range(number_of_clusters):
            ...
            features_correlation_per_cluster[i].append(
                sorted(result.items(), key=operator.itemgetter(1), reverse=True))

        # for cluster in features_correlation_per_cluster:
        #    print(cluster[0][0][0])
        # exit()
...
class Wrapper:
    ...
    def incremental_feature_select_using_correlation(subset_pmcs, model):
        ...
        # this dataframe will not have instrcutions and cycles
        
        df = ds.df_pmcs_avg[subset_pmcs].copy()
        #df['avg_instructions'] = ds.df_pmcs_avg['avg_instructions']
        df['0'] = ds.df_pmcs_avg['0']
        
        X = df.values
        last_column_name = list(ds.df_pmcs_avg.columns.values)[-2:]
        ...
        # REPRODUCIBLE since seed initial set
        model.set_params(estimator__random_state=seed)
        ...
        for j in range(MAX_PMCS_TO_SELECT):
            ...
            while i < len(df.columns):
                ...
                if i not in index_pmcs_selected:
                    ...
                    # concate with the pmcs selected before
                    X = np.concatenate((pmcs_selected, column), axis=1)
                    Y = array[:, [i for i in range(-2, 0, 1)]]
    ...
    def incremental_feature_select_using_importance(model):
        ...
        last_column_name = list(df.columns.values)[-2:]
        X = df.loc[:, df.columns.difference(last_column_name)]
...
class Filter:
    ...
    def get_features_correlated_with_target_sorted(df_pmcs_avg, method):
        ...
        '''
        df = df_pmcs_avg.copy()
        # As we will select them regardless, i removed
        df.drop(['0'], axis=1, inplace=True)
        last_column_name = list(df.columns.values)[-2:]
        correlation_matriz = df.corr(method)
        
        dic_params = {}
        for target in last_column_name:
            dic_params[target] = list(abs(correlation_matriz[last_column_name]).drop(last_column_name)[target])
            dic_params[target] = pd.DataFrame({target: dic_params[target]}, index=df.columns[:-2])
        ...
        for key in list(avg_dic_params.keys()):
            avg_dic_params[key] = avg_dic_params[key]/2

        avg_dic_params = pd.DataFrame(avg_dic_params, index=['0']).T
        avg_dic_params = avg_dic_params.sort_values('0', ascending=False)
    ...
    def get_features_HCA(number_of_clusters):
        ...
        for i in range(len(ds.df_pmcs.columns)):
            cluster_id = tree[i][0]  # get cluster identification
            # add the name of this pmcs in this cluster
            feat_per_cluster[cluster_id].append(ds.df_pmcs.columns[i])

        # Now we need to get the correlation
        dictionary = Filter.get_features_correlated_with_target_sorted(
            ds.df_pmcs_avg, method)

        for i in range(number_of_clusters):
            for j in range(len(feat_per_cluster[i])):
                # How do I assign a cluster
                feat_correlation[i].append(
                    dictionary[dictionary.keys()[0]].get(feat_per_cluster[i][j]))
...
class Tunning:
    ...
    def tunning_model(models, list_best_pmcs):
        ...
        for model_name, model in models:
            ...
            if model_name == 'DTR':
                param_grid = {'max_depth': [2, 8, 20, 50],
                              'max_features': ["auto", "sqrt", "log2"]}

            elif model_name == 'RFR':
                param_grid = {'n_estimators': [10, 30, 50, 80, 100, 120, 150], 'min_samples_split': [
                    2, 5, 10, 20], "bootstrap": [True, False]}

            elif model_name == 'GBR':
                param_grid = {'n_estimators': [30, 50, 80, 100, 150, 180, 200, 400, 600], 'learning_rate': [
                    0.1, 0.3, 0.5, 0.75, 0.9, 0.95], }

            elif model_name == 'XGBR':
                param_grid = {'n_estimators': [
                    30, 50, 80, 100, 120, 150, 200], 'colsample_bytree': [0.3, 0.5, 0.7, 0.9]}
            ...
            grid_ret = GridSearchCV(estimator=current_model, param_grid=param_grid,
                                    cv=k_fold, scoring='r2', verbose=False, n_jobs=-1)
...
class Accuracy:
    ...
    def calculate_accuracy(model_name, model, X, Y):
        ...
        while num_iterations < 10:  # Before i just increased k when it get minimum accuracy, but i decided to remove
            ...
            file_name = model_name+"_" + \
                str(num_iterations)+"_r2_"+str(round(current_r2, 2))+".png"
            title = sys.argv[3] + " " + model_name + "\n" + "R" + \
                str(chr(0x00B2)) + "=" + str(round(current_r2, 2))
...
if __name__ == '__main__':
    ...
    if sys.argv[3] == "FilterCorrelation":
        ...
        for i in range(len(corr_method)):
            ...
            subset_pmcs = []
            max = 0
            for pmc, corr in dic_params.iterrows():
                if pmc not in last_column_name:
                    if max == 30:  # get the best 30 features
                        break
                    subset_pmcs.append(pmc)
                    max += 1

            #########################################################################################

            #subset_pmcs = ['avg_cycles','avg_instructions'] + subset_pmcs
            best_pmcs, score, seeds = Wrapper.select_best_pmcs(models, subset_pmcs)
            ...
            for j in range(len(models)):
                ...
                logging.info("Model:%s best_pmcs:%s r2:%s",
                             models[j][0], best_pmcs[j], score[j])
                results.append(best_pmcs[j])

        Common.convert_list_to_df_and_save(
            results, "FilterCorrelation_Best_PMCs.csv")
        ...
        for i in range(len(corr_method)):
            ...
            for j in range(len(models)):
                row = []
                X, Y = Common.get_subset_features_and_target(
                    ds.df_pmcs_avg.copy(), best_pmcs[j])
                r2_list, avg, std = Accuracy.calculate_accuracy(
                    models[j][0], models[j][1], X, Y)
                logging.info("Model:%s r2:%s avg:%s std:%s",
                             models[j][0], r2_list, avg, std)
        ...
        Common.convert_list_to_df_and_save(
            results, "FilterCorrelation_Accuracy.csv")
        ...
        for i in range(len(corr_method)):
            ...
            logging.info("Correlation:%s", corr_method[i])
            features_per_cluster_sorted = HCA.get_features_per_cluster(
                30, corr_method[i])
            subset_features = []
            for cluster in features_per_cluster_sorted:
                subset_features.append(cluster[0][0][0])
            models = Models.get_models_initial_parameters()
            best_pmcs, score, seeds = Wrapper.select_best_pmcs(
                models, subset_features)

            # Common.plot_density_histogram_for_correlations(models,best_pmcs,corr_method[i])

            for j in range(len(models)):
                row = []
                print(models[j][0], best_pmcs[j], "R2:",
                      score[j], " Seed:", seeds[j])
                logging.info("Model:%s best_pmcs:%s r2:%s",
                             models[j][0], best_pmcs[j], score[j])
                X, Y = Common.get_subset_features_and_target(
                    ds.df_pmcs_avg.copy(), best_pmcs[j])
        ...
        Common.convert_list_to_df_and_save(
            results, "HCA_FilterCorrelation_Accuracy.csv")
        ...
        for j in range(len(models)):
            ...
            results.append(best_pmcs[j])
        Common.convert_list_to_df_and_save(
            results, "WrapperImportance_Best_PMCs.csv")
        ...
        for i in range(len(models)):
            ...
            X, Y = Common.get_subset_features_and_target(
                ds.df_pmcs_avg.copy(), best_pmcs[j])
            r2_list, avg, std = Accuracy.calculate_accuracy(
                models[i][0], models[i][1], X, Y)
            logging.info("Model:%s r2:%s avg:%s std:%s",
                         models[i][0], r2_list, avg, std)
            ...
            results.append(row)
        Common.convert_list_to_df_and_save(
            results, "WrapperImportance_Accuracy.csv")
